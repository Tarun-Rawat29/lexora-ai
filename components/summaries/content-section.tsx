...
const EmojiPoint = ({ point } : {
    ...
    return <MotionDiv
        ...
        className="group relative bg-linear-to-br from-gray-200/[0.08] to-gray-400/[0.03] p-4 rounded-2xl border border-gray-500/10 hover:shadow-lg translate-all"
    >
        ...
        className="space-y-4">
            {points.map((point, index) => {
                ...
                if (hasEmoji || isMainPoint) {
                    return (
                        <EmojiPoint point={point} key={`point-${point}`} />
                    );
                }
                return (
                    <RegularPoint point={point} key={`point-${point}`} />
                );
            })}
        </MotionDiv>
    )
